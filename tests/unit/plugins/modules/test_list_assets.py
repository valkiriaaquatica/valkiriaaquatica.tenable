...
import pytest
from ansible_collections.valkiriaaquatica.tenable.plugins.modules.list_assets import main
from ansible_collections.valkiriaaquatica.tenable.tests.unit.constants import BASE_MODULE_PATH


@pytest.fixture
def mock_module():
    with patch(BASE_MODULE_PATH + "list_assets.AnsibleModule", autospec=True) as mock:
        yield mock


def test_list_assets(mock_module):
    mock_module.return_value.params = {
        ...
        "all_fields": "full",
        "filters": [{"type": "aws_ec2_instance_id", "operator": "eq", "value": "i-123456789"}],
    }

    with patch(BASE_MODULE_PATH + "list_assets.run_module") as mock_run_module, patch(
        BASE_MODULE_PATH + "list_assets.add_custom_filters"
    ) as mock_add_custom_filters, patch(
        BASE_MODULE_PATH + "list_assets.build_query_parameters"
    ) as mock_build_query_parameters, patch(
        BASE_MODULE_PATH + "list_assets.handle_multiple_filters"
        ...
        assert called_args[1] == "workbenches/assets"
