...
import pytest
from ansible_collections.valkiriaaquatica.tenable.plugins.modules.list_networks import main
from ansible_collections.valkiriaaquatica.tenable.tests.unit.constants import BASE_MODULE_PATH


@pytest.fixture
def mock_module():
    with patch(BASE_MODULE_PATH + "list_networks.AnsibleModule", autospec=True) as mock:
        yield mock


def test_list_networks(mock_module):
    mock_module.return_value.params = {
        "access_key": "fake_access_key",
        "secret_key": "fake_secret_key",
        "filters": [{"type": "name", "operator": "eq", "value": "aws_network_1"}],
        "limit": 20,
        "offset": 0,
        "sort": "name:asc",
    ...
    with patch(BASE_MODULE_PATH + "list_networks.run_module") as mock_run_module, patch(
        BASE_MODULE_PATH + "list_networks.add_custom_filters"
    ) as mock_add_custom_filters, patch(
        BASE_MODULE_PATH + "list_networks.build_query_parameters"
    ) as mock_build_query_parameters, patch(
        BASE_MODULE_PATH + "list_networks.handle_special_filter"
